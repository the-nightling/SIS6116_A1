...
def plot_cities(
    cities_dict: dict[str, tuple[float, float]], test: bool = False
) -> None:
    """
    ...
    plt.scatter(*zip(*sorted_tuples), s=40, zorder=1)  # type: ignore
    plt.scatter(x=1.0, y=1.0)  # type: ignore
    plt.plot(*zip(*cities_dict.values()), "r--", zorder=0)  # type: ignore
    # if test is True:
...
def read_tsp_file_contents(filename: str) -> list[str]:
    """
...
class TSPParser:
    ...
    dimension: int
    tsp_file_contents: list[str] = []
    tsp_cities_dict: dict[str, tuple[float, float]] = {}
    ...
    def get_cities_dict(cls) -> None:
        ...
        for index in range(zero_index, zero_index + cls.dimension):
            parts: str = cls.tsp_file_contents[index].strip()
            city_coords_parts: list[str] = re.findall(r"[+-]?\d+(?:\.\d+)?", parts)
            print(city_coords_parts)
            cls.tsp_cities_dict[city_coords_parts[0]] = (
